...
func (e *jsiiProxy_EksCapability) validatePutConfigurationParameters(value *EksCapabilityConfiguration) error {
	if value == nil {
		return fmt.Errorf("parameter value is required, but nil was provided")
	}
...
func (e *jsiiProxy_EksCapability) validatePutTagsParameters(value interface{}) error {
	if value == nil {
		return fmt.Errorf("parameter value is required, but nil was provided")
	}
	switch value.(type) {
	...
	case []*EksCapabilityTags:
		value_ := value.([]*EksCapabilityTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*EksCapabilityTags; received %#v (a %T)", value, value)
		}
	}
