...
func (a *jsiiProxy_AccessanalyzerArchiveRule) validatePutFilterParameters(value interface{}) error {
	...
	case *map[string]*AccessanalyzerArchiveRuleFilter:
		value := value.(*map[string]*AccessanalyzerArchiveRuleFilter)
	...
	case map[string]*AccessanalyzerArchiveRuleFilter:
		value_ := value.(map[string]*AccessanalyzerArchiveRuleFilter)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *map[string]*AccessanalyzerArchiveRuleFilter; received %#v (a %T)", value, value)
		}
	}
