...
func (s *jsiiProxy_SsmMaintenanceWindow) validatePutTagsParameters(value interface{}) error {
	if value == nil {
		return fmt.Errorf("parameter value is required, but nil was provided")
	}
	switch value.(type) {
	case cdktn.IResolvable:
		// ok
	case *[]*SsmMaintenanceWindowTags:
		value := value.(*[]*SsmMaintenanceWindowTags)
	...
	case []*SsmMaintenanceWindowTags:
		value_ := value.([]*SsmMaintenanceWindowTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*SsmMaintenanceWindowTags; received %#v (a %T)", value, value)
		}
	}
