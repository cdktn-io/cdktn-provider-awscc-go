...
type EcsTaskDefinitionContainerDefinitionsPortMappings struct {
	...
	// The protocol that's used for the port mapping.
	//
	// Valid values are ``tcp`` and ``udp`` (case-sensitive). The default is ``tcp``. Amazon ECS treats any other specified value as ``tcp``. ``protocol`` is immutable in a Service Connect service. To update this field, you must delete and redeploy the service.
	//
