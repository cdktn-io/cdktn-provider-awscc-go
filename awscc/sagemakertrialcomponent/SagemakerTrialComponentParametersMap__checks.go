...
func (j *jsiiProxy_SagemakerTrialComponentParametersMap) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *map[string]*SagemakerTrialComponentParameters:
		val := val.(*map[string]*SagemakerTrialComponentParameters)
	...
	case map[string]*SagemakerTrialComponentParameters:
		val_ := val.(map[string]*SagemakerTrialComponentParameters)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *map[string]*SagemakerTrialComponentParameters; received %#v (a %T)", val, val)
		}
	}
