...
func (c *jsiiProxy_ConfigConnector) validatePutTagsParameters(value interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*ConfigConnectorTags:
		value := value.(*[]*ConfigConnectorTags)
	...
	case []*ConfigConnectorTags:
		value_ := value.([]*ConfigConnectorTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*ConfigConnectorTags; received %#v (a %T)", value, value)
		}
	}
