...
func (j *jsiiProxy_SsmCloudConnectorTagsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*SsmCloudConnectorTags:
		val := val.(*[]*SsmCloudConnectorTags)
	...
	case []*SsmCloudConnectorTags:
		val_ := val.([]*SsmCloudConnectorTags)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktn.IResolvable, *[]*SsmCloudConnectorTags; received %#v (a %T)", val, val)
		}
	}
