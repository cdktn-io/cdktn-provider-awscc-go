...
func (g *jsiiProxy_GlueIntegration) validatePutTagsParameters(value interface{}) error {
	...
	case cdktn.IResolvable:
		// ok
	case *[]*GlueIntegrationTags:
		value := value.(*[]*GlueIntegrationTags)
	...
	case []*GlueIntegrationTags:
		value_ := value.([]*GlueIntegrationTags)
		value := &value_
		for idx_cd4240, v := range *value {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter value[%#v]", idx_cd4240) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(value) {
			return fmt.Errorf("parameter value must be one of the allowed types: cdktn.IResolvable, *[]*GlueIntegrationTags; received %#v (a %T)", value, value)
		}
	}
